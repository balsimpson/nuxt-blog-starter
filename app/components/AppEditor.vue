...
<template>
  <div class="flex flex-col min-h-[520px] w-full min-w-0 overflow-hidden rounded-3xl border border-muted/40 bg-default shadow-sm">
    <UEditor
      v-slot="{ editor }"
      v-model="content"
      ...
      :ui="{
        base: 'p-4 max-w-full mx-auto focus:outline-none flex-1 min-w-0',
        content: 'p-4 prose prose-primary dark:prose-invert max-w-full break-words min-w-0'
      }"
    >
      <div class="sticky top-0 z-20 border-b border-muted bg-default/90 backdrop-blur min-w-0">
        <div class="flex flex-col gap-3 p-3 sm:p-4 min-w-0">
        ...
        <div class="border-t border-muted bg-default px-3 py-2">
          <UEditorToolbar
            :editor="editor"
            :items="toolbarItems"
            class="overflow-x-auto justify-center flex-wrap"
            :ui="{group: 'flex-wrap'}"
          />
        </div>
      </div>
